...
const CenterPanel: React.FC<{
  ...
  // Save logs to local storage/db
  useEffect(() => {
    logs.forEach(log => services.saveLog(log));
  }, [logs]);

  // Load logs for history or live view
  useEffect(() => {
    const loadHistory = async () => {
      if (isHistoryView) {
        ...
        // Reload current session logs instead of clearing
    ...
    <div className="bg-gradient-to-b from-[#053c3a] to-[#0f574f] rounded-xl p-4 shadow-lg text-white flex flex-col min-h-[720px]">
      ...
      <div className="mt-3 bg-gradient-to-b from-white/5 to-white/0 p-3 rounded-xl flex-1 flex flex-col">
          ...
          {connection === ConnectionType.Disconnected && (
            <div className="ml-auto flex gap-2">
              <button
                ...
                className="bg-transparent border border-white/20 px-3 py-1.5 text-sm rounded-lg hover:bg-white/10"
              >
                {uiStrings.bluetooth}
              </button>
              <button
        ...
        <div className="mt-3 flex items-center gap-3 text-sm text-white/90">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className={`p-1 rounded-md text-black text-xs ${!isHistoryView ? 'opacity-50 cursor-not-allowed' : ''}`}
            />
          </div>
        </div>
        ...
        <div ref={monitorRef} className="mt-3 bg-black/40 rounded-lg p-3 h-96 overflow-auto font-mono text-sm text-teal-200/90 leading-relaxed flex-1">
          {logs.map((log, index) => (
            <LogLine key={index} log={log} language={language} autoTranslate={autoTranslate} />
          ))}
        </div>

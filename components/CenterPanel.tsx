      ...
      (error) => {
        switch (error.code) {
          case error.PERMISSION_DENIED:
            appendLog("Location access denied by user.", "in");
            break;
          case error.POSITION_UNAVAILABLE:
            appendLog("Location information is unavailable.", "in");
            break;
          case error.TIMEOUT:
            appendLog("Location request timed out.", "in");
            break;
          default:
            appendLog("An unknown error occurred while fetching location.", "in");
        }
      },
      ...
      <div className="mt-3 bg-gradient-to-b from-white/5 to-white/0 p-3 rounded-xl flex-1 flex flex-col">
        ...
        <div className="mt-3 flex items-center gap-3 text-sm text-white/90">
          ...
          <div className={`relative inline-flex items-center rounded-md p-1 transition-all ${!isHistoryView ? 'bg-white/20 opacity-50 cursor-not-allowed' : 'bg-white/90 cursor-pointer'}`}>
            ...
            <input id="historyDate" type="date" value={historyDate} onChange={(e) => setHistoryDate(e.target.value)} disabled={!isHistoryView} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer disabled:cursor-not-allowed" />
          </div>
        </div>
        ...
        <div className="mt-3 flex gap-2 items-center flex-wrap">
          ...
          <button onClick={handleSearchHospitals} className="bg-rose-600 text-white px-4 py-2.5 rounded-lg font-bold hover:bg-rose-700 transition">

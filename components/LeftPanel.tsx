      ...
      <div className="mt-2 p-2.5 rounded-lg bg-teal-500/20 text-sm flex items-center justify-between gap-3 animate-fade-in">
        <div>
          <span className="font-bold text-white/90">
            Connected: {connection === ConnectionType.Bluetooth ? 'Bluetooth' : 'USB Serial'}
          </span>
          {connection === ConnectionType.Bluetooth && connectionDetails.rssi !== undefined && (
            <span className="text-white/70 text-xs">
              Signal: {connectionDetails.rssi} dBm ({rssiToQuality(connectionDetails.rssi)})
            </span>
          )}
          {connection === ConnectionType.Serial && (
            <span className="text-white/70 text-xs">
              {connectionDetails.baudRate && `Speed: ${connectionDetails.baudRate} bps`}
              {connectionDetails.usbVendorId && ` | Port: USB (VID:${connectionDetails.usbVendorId}, PID:${connectionDetails.usbProductId})`}
            </span>
          )}
        </div>
        <div className="flex items-center gap-2">
          {connection === ConnectionType.Bluetooth && (
            <button
              onClick={services.refreshBluetoothRssi}
              className="bg-transparent border border-white/20 p-1.5 rounded-lg hover:bg-white/10"
              title="Refresh Signal"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
                <path d="M13 3a9 9 0 00-9 9H1l3.89 3.89.07.14L9 12H6c0-3.87 3.13-7 7-7s7 3.13 7 7-3.13 7-7 7c-1.93 0-3.68-.79-4.94-2.06l-1.42 1.42A8.954 8.954 0 0013 21a9 9 0 009-9c0-4.97-4.03-9-9-9z" />
              </svg>
            </button>
          )}
          <button
            onClick={services.disconnect}
            className="bg-red-500/50 border border-red-400/50 px-3 py-1.5 text-sm rounded-lg hover:bg-red-500/80 transition"
          >
    ...
    <div className="bg-gradient-to-b from-[#053c3a] to-[#0f574f] rounded-xl p-4 shadow-lg text-white flex flex-col min-h-[720px]">
      <div className="flex items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          <select
            ...
            className="bg-white/10 p-2 rounded-lg border-0 text-white focus:outline-none focus:ring-2 focus:ring-teal-400"
          >
      ...
      <div className="mt-3 bg-gradient-to-b from-white/5 to-white/0 p-3 rounded-xl flex-1 flex flex-col">
        ...
        <div className="mt-3 flex items-center gap-3 text-sm text-white/90">
          ...
          <div
            className={`relative inline-flex items-center rounded-md p-1 transition-all ${!isHistoryView ? 'bg-white/20 opacity-50 cursor-not-allowed' : 'bg-white/90 cursor-pointer'}`}
          >
            <span className={`text-xs pointer-events-none ${!isHistoryView ? 'text-white/70' : 'text-black'}`}>
              {historyDate}
            </span>
            <input
              id="historyDate"
        ...
        {aiReport && (
          <div className="mt-3 bg-purple-900/30 border border-purple-400/50 rounded-lg p-3 animate-fade-in">
            <div className="flex justify-between items-center mb-2">
              <div className="flex items-center gap-2 font-bold text-purple-200">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M5 2a1 1 0 00-1 1v14a1 1 0 001 1h10a1 1 0 001-1V3a1 1 0 00-1-1H5zm0 2h10v12H5V4zm2 5a1 1 0 011-1h4a1 1 0 110 2H8a1 1 0 01-1-1zm0 4a1 1 0 011-1h4a1 1 0 110 2H8a1 1 0 01-1-1z" clipRule="evenodd" />
                </svg>
            ...
            <div className="text-sm text-purple-100/90 whitespace-pre-wrap font-mono bg-black/20 p-2 rounded-md max-h-40 overflow-auto">
              {aiReport}
            </div>
          </div>
        )}
        ...
        <div ref={monitorRef} className="mt-3 bg-black/40 rounded-lg p-3 h-96 overflow-auto font-mono text-sm text-teal-200/90 leading-relaxed flex-1">
          {logs.map((log, i) => (
            <LogLine key={i} log={log} language={language} autoTranslate={autoTranslate} />
          ))}
        </div>
        ...
        <div className="mt-3 flex gap-2 items-center">
          <input
            ...
            className="flex-1 p-2.5 rounded-lg border-0 bg-white/10 text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-teal-400"
          />
          <button onClick={handleSend} className="bg-teal-500 text-white px-4 py-2.5 rounded-lg font-bold hover:bg-teal-600">
            {uiStrings.send}
          </button>
          <button
            onClick={handleAnalyze}
            disabled={isAnalyzing || logs.length === 0}
            title={uiStrings.analyze}
            className="bg-purple-600 text-white p-2.5 rounded-lg font-bold hover:bg-purple-700 transition disabled:bg-purple-400/80 disabled:cursor-not-allowed"
          >
            {isAnalyzing ? (
          ...
          <button
            ...
            className="bg-rose-600 text-white px-4 py-2.5 rounded-lg font-bold hover:bg-rose-700 transition"
          >
